...
class ThuocModelView(ModelView):
    can_export= True
    column_labels = dict(tenthuoc="Tên thuốc", soluong="Số lượng", dongia="Đơn giá", trangthai="Trạng thái", CachDung="Cách dùng", DonVi="Đơn vị")
    column_searchable_list = ("tenthuoc", "dongia")
    page_size = 10   #phân trang

class DonViModelView(ModelView):
    column_labels = dict(donvi="Đơn vị thuốc")
    page_size = 10   #phân trang

class CachDungModelView(ModelView):
    column_labels = dict(cachdung="Cách dùng thuốc")
    page_size = 10   #phân trang
